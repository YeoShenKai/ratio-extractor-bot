...
app = Flask(__name__)
app.secret_key = 'NO_ERRORS_PLEASE'
...
def index(): #Fix url upon publish
    ...
    try:
        ...
        for dependent, indep_eqn_corr in besteqns_predictions[0].items(): #change array to list, so that it can be stored in sessions
            to_change = indep_eqn_corr[1]
            to_change = to_change.tolist()
            indep_eqn_corr[1] = to_change
        ...
        session['besteqns_predictions'] = besteqns_predictions 
        session['besteqns'] = besteqns
        session['predictions'] = predictions
        session['user_inputs'] = user_input_values 
...
def plots():
    try: 
        ...
        if plot_selection:
            besteqns_predictions = session['besteqns_predictions']
            user_input_values = session['user_inputs']
        ...
        return f"<p> {e} </p> {render_template('plotpage-error.html')}"
